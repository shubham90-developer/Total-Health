...
import { useGetCashExpensesQuery, useDeleteExpenseMutation } from '@/services/expenseApi'
...
const CashExpenseList = () => {
  ...
  const { data: expensesRes, isLoading, error, refetch } = useGetCashExpensesQuery(
  ...
  // Debug logging
  React.useEffect(() => {
    console.log('Cash Expenses Query Params:', queryParams)
    console.log('Cash Expenses Data:', expensesRes)
    console.log('Cash Expenses Error:', error)
    console.log('Cash Expenses Loading:', isLoading)
  }, [expensesRes, error, isLoading, queryParams])

  const expenses = useMemo(() => {
    const data = expensesRes?.data || []
    console.log('Cash expenses array:', data)
    console.log('Cash expenses count:', data.length)
    return data
  }, [expensesRes])
      ...
      <Col xl={12}>
        <Card>
          <CardHeader className="d-flex flex-wrap justify-content-between align-items-center gap-2">
            ...
            <Link href="/expenses/add-expense?paymentMethod=Cash" className="btn btn-lg btn-primary">
              + Add Cash Expense
            </Link>
          </CardHeader>
              ...
              <table className="table align-middle mb-0 table-hover table-centered table-bordered">
                ...
                <tbody>
                  ...
                  {!isLoading && !error && filteredExpenses.length === 0 && (
                    <tr>
                      <td colSpan={11} className="text-center py-4">No cash expenses found</td>
                    </tr>
                  )}
                  {error && (
                    <tr>
