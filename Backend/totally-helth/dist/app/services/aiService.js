...
class AIService {
     ...
     */
    generateDescription(name, context) {
     ...
     */
    improveDescription(text, context) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let prompt = '';
                if (context === 'restaurant offer') {
     ...
     */
    generateDocument(documentType, platformName) {
        return __awaiter(this, void 0, void 0, function* () {
            let prompt = '';
            switch (documentType) {
                case 'privacy-policy':
     ...
     */
    generateHotelMenuChat(hotel_1, userMessage_1) {
        return __awaiter(this, arguments, void 0, function* (hotel, userMessage, conversationHistory = []) {
            try {
                    ...
                    categories: hotel.menuCategories.map((category) => ({
                        ...
                        items: category.items.map((item) => {
                            ...
                            return ({
                                ...
                                price: item.price,
                                isVeg: (_a = item.itemType) === null || _a === void 0 ? void 0 : _a.includes('Veg'),
                                isNonVeg: (_b = item.itemType) === null || _b === void 0 ? void 0 : _b.includes('Non-Veg'),
                                isEgg: (_c = item.itemType) === null || _c === void 0 ? void 0 : _c.includes('Contains Egg'),
                ...
                if (conversationHistory.length > 0) {
                    conversationContext = '\n\nPrevious conversation:\n' +
                        conversationHistory.slice(-4).map((msg) => `${msg.sender === 'user' ? 'Customer' : 'AI'}: ${msg.message}`).join('\n');
                }
     ...
     */
    generateFoodSuggestions(menuItems_1) {
        return __awaiter(this, arguments, void 0, function* (menuItems, limit = 10) {
            try {
      ...
      Focus on variety, nutritional value, taste, and popularity. Ensure the response is valid JSON only.`;
                ...
                const finalSuggestions = aiRecommendations.map(aiItem => {
                    const originalItem = menuItems.find(item => item.title.toLowerCase().includes(aiItem.title.toLowerCase()) ||
                        aiItem.title.toLowerCase().includes(item.title.toLowerCase()));
                    if (originalItem) {
                        return Object.assign(Object.assign({}, originalItem), { aiExplanation: aiItem.aiExplanation, healthScore: aiItem.healthScore, popularityReason: aiItem.popularityReason, isAIRecommended: true });
                    }
                    return null;
                }).filter(Boolean);
                ...
                if (finalSuggestions.length < limit) {
                    ...
                    const additionalItems = menuItems
                        ...
                        .map(item => (Object.assign(Object.assign({}, item), { aiExplanation: "This dish stands out for its exceptional taste and quality ingredients.", healthScore: item.isVeg ? 7 : 5, popularityReason: "Recommended by our culinary experts", isAIRecommended: true })));
                ...
                return menuItems.slice(0, limit).map(item => (Object.assign(Object.assign({}, item), { aiExplanation: "This delicious dish is crafted with care and loved by our customers for its authentic flavors.", healthScore: item.isVeg ? 7 : 5, popularityReason: item.isHighlyReordered ? "Customer favorite" : "Chef's recommendation", isAIRecommended: true })));
            }
        });
    }
}
